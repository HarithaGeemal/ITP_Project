...
const SEDailyReports = () => {
    const { dailyReports, addDailyReport, updateDailyReport, deleteDailyReport, projects } = useSEContext();
    ...
    const handleSubmit = async (e) => {
        ...
        let error;
        if (currentReport) error = await updateDailyReport(currentReport.id, formData);
        else error = await addDailyReport(formData);
    ...
    const handleDelete = async (id, projectId) => {
        ...
        const error = await deleteDailyReport(id, projectId);
                ...
                {filtered.map(report => (
                    <div key={report.id} className="bg-white rounded-xl shadow-sm border border-concrete-light p-5 flex flex-col hover:shadow-md transition-shadow">
                        ...
                        <div className="mt-auto pt-3 border-t border-concrete-light flex justify-end space-x-3">
                            ...
                            <button onClick={() => handleDelete(report.id, report.projectId)} className="text-red-500 hover:text-red-700 text-sm font-medium">Delete</button>
                        </div>
                    </div>
                ))}
                {filtered.length === 0 && (
